...
void PathPlan::odomCallback(const nav_msgs::OdometryConstPtr& odomMsg)
{
  ...
  // only check the wall distribution when the robot is near the center of the cell, to prevent
  // walls from being wrongly identified
  if(pos_x_-floor(pos_x_) > WALL_BORDER/2 && pos_x_-floor(pos_x_) < 1-WALL_BORDER/2 &&
      pos_y_-floor(pos_y_) > WALL_BORDER/2 && pos_y_-floor(pos_y_) < 1-WALL_BORDER/2){
    checkWall();
...
void PathPlan::checkWall()
{
  ...
  if(dist_north_ < WALL_DETECT_DIST && !hasWall(pos_x_int, pos_y_int, NORTH)){
    wall_changed_ = true;
    setWall(pos_x_int, pos_y_int, NORTH);
  } 
  if(dist_north_ > OPEN_DETECT_DIST && hasWall(pos_x_int, pos_y_int, NORTH)){
    wall_changed_ = true;
    removeWall(pos_x_int, pos_y_int, NORTH);
  }
  
  if(dist_east_ < WALL_DETECT_DIST && !hasWall(pos_x_int, pos_y_int, EAST)){
    wall_changed_ = true;
    setWall(pos_x_int, pos_y_int, EAST);
  } 
  if(dist_east_ > OPEN_DETECT_DIST && hasWall(pos_x_int, pos_y_int, EAST)){
    wall_changed_ = true;
    removeWall(pos_x_int, pos_y_int, EAST);
  }
  
  if(dist_south_ < WALL_DETECT_DIST && !hasWall(pos_x_int, pos_y_int, SOUTH)){
    wall_changed_ = true;
    setWall(pos_x_int, pos_y_int, SOUTH);
  } 
  if(dist_south_ > OPEN_DETECT_DIST && hasWall(pos_x_int, pos_y_int, SOUTH)){
    wall_changed_ = true;
    removeWall(pos_x_int, pos_y_int, SOUTH);
  }
  
  if(dist_west_ < WALL_DETECT_DIST && !hasWall(pos_x_int, pos_y_int, WEST)){
    wall_changed_ = true;
    setWall(pos_x_int, pos_y_int, WEST);
  } 
  if(dist_west_ > OPEN_DETECT_DIST && hasWall(pos_x_int, pos_y_int, WEST)){
    wall_changed_ = true;
...
void PathPlan::path_plan_alg()
{
  ...
  std::queue<pair<int, int>> reached_queue;
  
  reached_queue.push(pair<int, int>(GOAL_X, GOAL_Y));
  ...
  while(nVisited < nCells){
    ...
    for(int i = 0; i<queue_length; i++){
      ...
      if(visited_map.at(tmp.first, tmp.second) == 0){
        double weight = path_map_(tmp.first, tmp.second) + manhattenDist(tmp, pair<int, int>(GOAL_X, GOAL_Y));
      	if(weight < min_dist){
      		min_dist = weight;
      		node = tmp;
      	}
    ...
    */
	...
	if(!hasWall(node.first, node.second, direction)){
	  ...
	  if(!is_queued(x_queue, y_queue)){
	    ...
	    reached_queue.push(pair<int, int>(x_queue, y_queue));
...
void PathPlan::setNextDestCell()
{
  ...
  if(pos_x_int == GOAL_X && pos_y_int == GOAL_Y){
    target_x_ = GOAL_X;
    target_y_ = GOAL_Y;
    if(abs(pos_x_-GOAL_X-0.5) < 0.02 && abs(pos_y_-GOAL_Y-0.5) < 0.02) {
      goal_reached_ = GOAL_REACH;
      ROS_INFO("Goal Reach!, X:%d, Y:%d", GOAL_X, GOAL_Y);
    } else {
      goal_reached_ = CELL_REACH;
    }
    return;
  }
  ...
  neighbor_value_(NORTHEAST) = getPathMapValue(pos_x_int+1, pos_y_int+1);
  neighbor_value_(SOUTHEAST) = getPathMapValue(pos_x_int+1, pos_y_int-1);
  neighbor_value_(SOUTHWEST) = getPathMapValue(pos_x_int-1, pos_y_int-1);
  neighbor_value_(NORTHWEST) = getPathMapValue(pos_x_int-1, pos_y_int+1);
  ...
  //NORTHEAST
  if(pos_x_int+1<GRID_SIZE && pos_y_int+1<GRID_SIZE &&
          !hasWall(pos_x_int, pos_y_int, NORTH) && !hasWall(pos_x_int, pos_y_int, EAST) &&
          !hasWall(pos_x_int+1, pos_y_int+1, SOUTH) && !hasWall(pos_x_int+1, pos_y_int+1, WEST)){
    //find the path with minimum path value and without a obstacle
    if(neighbor_value_(NORTHEAST) <= min_path_value){
      min_path_value = neighbor_value_(NORTHEAST);
      min_heading = NORTHEAST;
    }
  }

  //SOUTHEAST
  if(pos_x_int+1<GRID_SIZE && pos_y_int-1>=0 && 
          !hasWall(pos_x_int, pos_y_int, SOUTH) && !hasWall(pos_x_int, pos_y_int, EAST) &&
          !hasWall(pos_x_int+1, pos_y_int-1, NORTH) && !hasWall(pos_x_int+1, pos_y_int-1, WEST)){
    //find the path with minimum path value and without a obstacle
    if(neighbor_value_(SOUTHEAST) <= min_path_value){
      min_path_value = neighbor_value_(SOUTHEAST);
      min_heading = SOUTHEAST;
    }
  }

  //SOUTHWEST
  if(pos_x_int-1>=0 && pos_y_int-1>=0 &&
          !hasWall(pos_x_int, pos_y_int, SOUTH) && !hasWall(pos_x_int, pos_y_int, WEST) &&
          !hasWall(pos_x_int-1, pos_y_int-1, NORTH) && !hasWall(pos_x_int-1, pos_y_int-1, EAST)){
    //find the path with minimum path value and without a obstacle
    if(neighbor_value_(SOUTHWEST) <= min_path_value){
      min_path_value = neighbor_value_(SOUTHWEST);
      min_heading = SOUTHWEST;
    }
  }

  //NORTHWEST
  if(pos_x_int-1>=0 && pos_y_int+1<GRID_SIZE &&
          !hasWall(pos_x_int, pos_y_int, NORTH) && !hasWall(pos_x_int, pos_y_int, WEST) &&
          !hasWall(pos_x_int-1, pos_y_int+1, SOUTH) && !hasWall(pos_x_int-1, pos_y_int+1, EAST)){
    //find the path with minimum path value and without a obstacle
    if(neighbor_value_(SOUTHWEST) <= min_path_value){
      min_path_value = neighbor_value_(SOUTHWEST);
      min_heading = SOUTHWEST;
  ...
  if(min_heading < 0){ //haven't find correct path, then go back
    ...
    if(min_heading == NORTHEAST){target_x_ = pos_x_int+1; target_y_ = pos_y_int+1;}
    if(min_heading == SOUTHEAST){target_x_ = pos_x_int+1; target_y_ = pos_y_int-1;}
    if(min_heading == SOUTHWEST){target_x_ = pos_x_int-1; target_y_ = pos_y_int-1;}
    if(min_heading == NORTHWEST){target_x_ = pos_x_int-1; target_y_ = pos_y_int+1;}
